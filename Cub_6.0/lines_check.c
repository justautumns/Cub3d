...
/*   Updated: 2024/09/03 04:17:22 by mtrojano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	vertical_line_check(t_data *d)
{
	...
	{
		d->ray_x = (((int)d->player_x / TILE_SIZE) * TILE_SIZE) - 0.001; // Left
		d->ray_y = (d->player_x - d->ray_x) * _tan + d->player_y; // + 0.01; // maybe not/ + affects south
	...
	{
		d->ray_x = (((int)d->player_x / TILE_SIZE) * TILE_SIZE) + TILE_SIZE; // + 0.01;
		d->ray_y = (d->player_x - d->ray_x) * _tan + d->player_y; // + 0.0001;
		d->x_offset = TILE_SIZE;
		d->y_offset = -d->x_offset * _tan;
		d->ray_direction = 3;
	}
...
void	horizontal_line_check(t_data *d)
{
	...
	if (d->ray_angle > PI)
	{
		d->ray_y = (((int)d->player_y / TILE_SIZE) * TILE_SIZE) - 0.001; // Up
		d->ray_x = (d->player_y - d->ray_y) * _tan + d->player_x; // + 0.001;
	...
	{
		d->ray_y = (((int)d->player_y / TILE_SIZE) * TILE_SIZE) + TILE_SIZE; // + 0.001; // +/- fucks both ways up(?);
		d->ray_x = (d->player_y - d->ray_y) * _tan + d->player_x; // + 0.0001;
		d->y_offset = TILE_SIZE;
		d->x_offset = -d->y_offset * _tan;
		d->ray_direction = 1;
	}
